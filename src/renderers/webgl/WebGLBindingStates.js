class WebGLBindingStates {
	...
	constructor( gl, extensions, attributes, capabilities ) {
		...
		const maxVertexAttributes = this.gl.getParameter( this.gl.MAX_VERTEX_ATTRIBS );
		this.maxVertexAttributes = maxVertexAttributes;

		const extension = capabilities.isWebGL2 ? null : extensions.get( 'OES_vertex_array_object' );
		this.extension = extension;
		const vaoAvailable = capabilities.isWebGL2 || extension !== null;
		this.vaoAvailable = vaoAvailable;

		const bindingStates = {};
		this.bindingStates = bindingStates;

		const defaultState = this.createBindingState( null );
	...
	enableAttributeAndDivisor( attribute, meshPerAttribute ) {
		...
		if ( attributeDivisors[ attribute ] !== meshPerAttribute ) {
			...
			const extension = this.capabilities.isWebGL2 ? this.gl : this.extensions.get( 'ANthis.glE_instanced_arrays' );
